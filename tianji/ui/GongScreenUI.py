...
class GongScreen(BoxLayout):
    def __init__(self, gong_obj=None, screen_obj=None, **kwargs):
        ...
        m.add_widget(StarListScreen(star_list=self.other_info, orientation=dui_qi_fang_shi,
                                    event=self.on_touch_up_envent(gong_obj.location)))
        ...
        for star in star_list:
            ...
            for temp in ["化权", "化科", "化禄", "化忌"]:
                if temp in star:
                    ...
                    button2 = Button(text=temp[1], size=(40, 22), size_hint=(None, None), bold=True)
                    button.info = res[1].info
                    button.level = res[1].level
                    ...
                    button2.background_color = color_config.get(temp)


                    # if temp == "化忌":
                    #     button2.color = (1, 1, 1, 1)
                    #     button2.font_size += 1
                    #     button2.background_color = "#EE545E"  # "#5EB125"
